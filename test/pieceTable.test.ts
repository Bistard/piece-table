...
import { Pair, Random } from 'test/helpers';
import { Position, EndOfLineType, IPieceNode, IPieceTable, RBColor } from 'src/index';
import { NULL_NODE } from 'src/pieceTable';
import { TextBuffer } from 'src/textBuffer';
import { PieceTableBuilder } from 'src/pieceTableBuilder';

class TestPieceTableBuilder extends PieceTableBuilder {
    constructor() {
        super();
    }
...
suite('PieceTable-test - content APIs', () => {
    ...
    const offsetPositionCheck = function (table: IPieceTable, lineInfo: Pair<number, number>[]): void {
        ...
        for (lineNumber = 0; lineNumber < lineInfo.length; lineNumber++) {
            ...
            for (let offset = 0; offset < rawLineLength; offset++) {
                ...
                assert.deepStrictEqual(table.getPositionAt(textOffset), new Position(lineNumber, offset));
            }
        }
    ...
    test('getOffsetAt / getPostionAt / getLineLength', () => {
        let surrogates = '😁';
        let table = buildPieceTable(['Hello ', 'World.\nMy name is Chris\r\n', 'I started this project \n', 'when I was first year in university.\r\nI wish whoever\r', ' read this line of code\r\n', 'take care of yourself' + surrogates.charAt(0), surrogates.charAt(1) + ' and have a \n', 'nice day!\n'], false);
        assert.deepStrictEqual(table.getPositionAt(table.getOffsetAt(0, 0)), new Position(0, 0));
        assert.deepStrictEqual(table.getPositionAt(table.getOffsetAt(1, 0)), new Position(1, 0));
        assert.deepStrictEqual(table.getPositionAt(table.getOffsetAt(1, 6)), new Position(1, 6));
        assert.deepStrictEqual(table.getPositionAt(table.getOffsetAt(2, 0)), new Position(2, 0));
        assert.deepStrictEqual(table.getPositionAt(table.getOffsetAt(3, 0)), new Position(3, 0));
        assert.deepStrictEqual(table.getPositionAt(table.getOffsetAt(4, 0)), new Position(4, 0));
        PieceTableTester.assertPieceTable(table);
        ...
        assert.deepStrictEqual(table.getPositionAt(1000), new Position(9, 15));
        PieceTableTester.assertPieceTable(table);
    });
